...
public class ContributeCorrectedDimensionVector {
        ...
        newDimVectors.entrySet().stream()
                ...
                .forEach(
                        e -> {
                            entityGenerator.unitOfWork(
                                    tool -> {
                                        PrintStream out = new PrintStream(new IndentedOutputStream(System.out, "  # "));
                                        Unit unit = e.getKey();
                                        String correctedDimensionVector = e.getValue();
                                        out.println(String.format("%s has incorrect dimension vector:\n\t%s, which should be \n\t%s", unit.getIriAbbreviated(), unit.getDimensionVectorIri().orElse("[no dim vector]"), correctedDimensionVector));
                                        ...
                                        out.println(String.format("the unit has these quantityKinds: %s", quantityKindSet.stream().map(QuantityKind::getIriLocalname).collect(
                                                        Collectors.joining(", "))));

                                            for (QuantityKind qk: quantityKindSet) {
                                                List<Unit> otherUnits = getUnitsAssociatedWithQuantityKind(qk);
                                                otherUnits.remove(unit);
                                                out.println(String.format(
                                                                "\tquantityKind %s, has %d other units associated",
                                                                qk.getIriLocalname(),
                                                                otherUnits.size()));
                                                if (!(otherUnits.isEmpty() || qk.getIriLocalname().equals("Unknown"))) {
                                                    for (Unit otherUnit : otherUnits) {
                                                        out.println(String.format("\t\t%-40s %s",
                                                                        otherUnit.getIriAbbreviated(),
                                                                        otherUnit.getDimensionVectorIri()
                                                                                        .orElse("[no dim vector]")));
                                                    }
                                                }
                                            }
                                        List<QuantityKind> fittingQuantityKinds = getQuantityKindsByDimensionVector(correctedDimensionVector);
                                        List<Node<QuantityKind>> quantityKindForest = QuantityKindTree.makeSkosBroaderForest(fittingQuantityKinds);
                                        List<Node<Object>> quantityKindsWithUnits = QuantityKindTree.addAssociatedUnitsToQuantityKindForest(quantityKindForest);

                                        out.println(String.format("quantity kinds that fit the corrected dimension vector %s, and associated units:", correctedDimensionVector));
                                        for (Node<Object> root: quantityKindsWithUnits){
                                            StringBuilder stringBuilder = new StringBuilder();
                                            FormattingNodeVisitor formatter = new FormattingNodeVisitor<Object>(stringBuilder, (sb, node) -> {
                                                sb.append((node.getData() instanceof Unit)
    ...
    private static List<Unit> getUnitsAssociatedWithQuantityKind(QuantityKind quantityKind){
        return Qudt.allUnits().stream().filter(u -> u.getQuantityKinds().contains(quantityKind)).collect(Collectors.toList());
    }

    private static List<QuantityKind> getQuantityKindsByDimensionVector(String dimensionVectorIri){
        return Qudt.allQuantityKinds().stream().filter(qk -> qk.getDimensionVectorIri().map(dv -> dv.equals(dimensionVectorIri)).orElse(false)).collect(
                        Collectors.toList());
    }
    ...
    private static List<Unit> getUnitsByDimensionVector(String dimensionVectorIri){
        return Qudt.allUnits().stream().filter(u -> u.getDimensionVectorIri().map(dv -> dv.equals(dimensionVectorIri)).orElse(false)).collect(
                        Collectors.toList());
    }
